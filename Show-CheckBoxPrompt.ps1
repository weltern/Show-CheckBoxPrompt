#region Function Show-CheckBoxPrompt
Function Show-CheckBoxPrompt {
    <#
.SYNOPSIS
...
Create a custom checkbox pop up along with any combination of Left, Middle or Right buttons can be displayed. The return value of the button clicked by the user is the button text specified and the return value of the check boxes is the text specified.
...
.PARAMETER CheckBoxOptions

Specifies an array of strings to be displayed as checkboxes in the prompt.
...
This function is a custom extension made specifically for PSAppDeployToolkit. Copy and paste code into the main section of the AppDeployToolkitExtensions.ps1 file
...
#>
    ...
    Process {
        ...
        If ($deployModeSilent) {
            Write-Log -Message "Bypassing Show-CheckBoxPrompt [Mode: $deployMode]. Message:$Message" -Source ${CmdletName}
            Return
        }
        ...
        # Create a FlowLayoutPanel for checkboxes
        $flowPanelCheckBoxes = New-Object -TypeName 'System.Windows.Forms.FlowLayoutPanel'
        ...
        if ($CheckBoxOptions) {
            $CheckBoxes = @()
            $CheckBoxOptions | ForEach-Object {
                $CheckBox = New-Object -TypeName 'System.Windows.Forms.CheckBox'
                $CheckBox.Text = $_
                $CheckBox.AutoSize = $true
                $CheckBox.Margin = New-Object -TypeName 'System.Windows.Forms.Padding' -ArgumentList (10, 5, 10, 5)
                $CheckBox.Anchor = 'Left'
                $CheckBoxes += $CheckBox
                $flowPanelCheckBoxes.Controls.Add($CheckBox)
            }
            $flowPanelCheckBoxes.MinimumSize = $DefaultControlSize
            $flowPanelCheckBoxes.MaximumSize = $DefaultControlSize
            $flowPanelCheckBoxes.ClientSize = $DefaultControlSize
            $flowPanelCheckBoxes.AutoSize = $true
            $flowPanelCheckBoxes.Margin = New-Object -TypeName 'System.Windows.Forms.Padding' -ArgumentList (0, 0, 0, 0)
            $flowPanelCheckBoxes.Padding = New-Object -TypeName 'System.Windows.Forms.Padding' -ArgumentList (10, 10, 10, 10)
            $flowPanelCheckBoxes.WrapContents = $true
            $flowPanelCheckBoxes.FlowDirection = 'TopDown'
            $flowPanelCheckBoxes.Anchor = 'Top,Left'
        }
        ...
        If ($CheckBoxOptions) {
            $flowLayoutPanel.Controls.Add($flowPanelCheckBoxes)
        }
        ...
        $checkedOptions = $CheckBoxes | Where-Object { $_.Checked } | ForEach-Object { $_.Text }

        Switch ($result) {
            'Yes' {
                Write-Output -InputObject ($buttonRightText, $checkedOptions)
            }
            'No' {
                Write-Output -InputObject ($buttonLeftText, $checkedOptions)
            }
            'Ignore' {
                Write-Output -InputObject ($buttonMiddleText, $checkedOptions)
            }
            'Abort' {
